...
void BNO055_Initialise(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address)
{
	...
	BNO055_Set_Operation_Mode(hi2c,huart,Module_Address,Mod_Configurare_NDOF);

	// Setare Mod Configurare Power
	uint8_t Mod_Configurare_Power = NORMAL_MODE;
	BNO055_Write_Reg(hi2c,huart,BNO055_PWR_MODE,&Mod_Configurare_Power,Module_Address,1);
...
void BNO055_Read_Quaternion_W(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer[2];
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_W_LSB,&Buffer[0],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_W_MSB,&Buffer[1],Module_Address,1);
...
void BNO055_Read_Quaternion_X(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer[2];
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_X_LSB,&Buffer[0],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_X_MSB,&Buffer[1],Module_Address,1);
...
void BNO055_Read_Quaternion_Y(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer[2];
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Y_LSB,&Buffer[0],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Y_MSB,&Buffer[1],Module_Address,1);
...
void BNO055_Read_Quaternion_Z(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer[2];
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Z_LSB,&Buffer[0],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Z_MSB,&Buffer[1],Module_Address,1);
...
void BNO055_Read_Quaternion_WXYZ(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_W_LSB,&Buffer[0],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_W_MSB,&Buffer[1],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_X_LSB,&Buffer[2],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_X_MSB,&Buffer[3],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Y_LSB,&Buffer[4],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Y_MSB,&Buffer[5],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Z_LSB,&Buffer[6],Module_Address,1);
	BNO055_Read_Reg(hi2c,huart,BNO055_QUA_DATA_Z_MSB,&Buffer[7],Module_Address,1);
...
void BNO055_Convert_Quat_to_Euler(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	IMU_Data -> Yaw = IMU_Data -> Yaw - IMU_Data -> Yaw_Offset + 90; // Atentie !!! Trebuie sa te pozitionezi in directia Nordului Magnetic pentru a evita Gimbal's Lock!!!
	IMU_Data -> Roll = IMU_Data -> Roll + 45;
	IMU_Data -> Pitch = IMU_Data -> Pitch + 90;

	if((IMU_Data -> Yaw >= 180) && (IMU_Data -> Yaw <= 500))
		{
			IMU_Data -> Yaw = 180;
		}
	if((IMU_Data -> Yaw <= 0) || (IMU_Data -> Yaw > 500) )
		{
			IMU_Data -> Yaw = 0;
		}
	if((IMU_Data -> Roll >= 225) && (IMU_Data -> Roll <= 360))
		{
			IMU_Data -> Roll = 225;
		}
	if((IMU_Data -> Roll <= 0) || (IMU_Data -> Roll > 360) )
		{
			IMU_Data -> Roll = 0;
		}
}
...
/*
 * Initial, sunt necesare cateva masurari(aproximativ 5) fara Offset pentru a masura exact directia fata de Campul Magnetic al Pamantului si a seta un Offset cat mai exact, astfel incat,
 * indiferent de pozitia absoluta a Nordului Magnetic, cea relativa(cea initiala a senzorului) sa fie 0.
 */
void BNO055_Convert_Quat_to_Euler_WOffset(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	float pi = 3.14159265359;
	IMU_Data -> Roll = (atan2(2*((IMU_Data -> w)*(IMU_Data -> x)+(IMU_Data -> y)*(IMU_Data -> z)),(1-2*((IMU_Data -> x)*(IMU_Data -> x)+(IMU_Data -> y)*(IMU_Data -> y)))))*180/pi;
	IMU_Data -> Pitch = (asin(2*((IMU_Data -> w)*(IMU_Data -> y)-(IMU_Data -> x)*(IMU_Data -> z))))*180/pi;
...
void BNO055_Set_Offset_Yaw(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	for(int i = 0;i<=5;i++)
	{
	BNO055_Read_Quaternion_WXYZ(hi2c,huart,Module_Address,IMU_Data);
	BNO055_Convert_Quat_to_Euler_WOffset(hi2c,huart,Module_Address,IMU_Data);
	HAL_Delay(50);
	}
	IMU_Data -> Yaw_Offset = IMU_Data -> Yaw;
}
/*
...
void BNO055_Read_Calib_Stat(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer;
	BNO055_Read_Reg(hi2c,huart,BNO055_CALIB_STAT,&Buffer,Module_Address,1);
...
void BNO055_Read_Calib_Data(I2C_HandleTypeDef *hi2c,UART_HandleTypeDef *huart,uint8_t Module_Address,BNO055_Data* IMU_Data)
{
	...
	uint8_t Buffer[22];
	BNO055_Read_Reg(hi2c,huart,BNO055_ACC_OFFSET_X_LSB,&Buffer[0],Module_Address,22);
