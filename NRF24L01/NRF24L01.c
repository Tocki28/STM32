...
void NRF24L01_Init_TxAA (TIM_HandleTypeDef *htim)
{
	...
	NRF24L01_WriteReg(CONFIG, 0b01001010);  // Configure conform caietel;
...
void NRF24L01_Transmit (uint8_t *Tx_Data)
{
	...
	// Sending the PD
	HAL_SPI_Transmit(NRF24_SPI, Tx_Data, sizeof(Tx_Data), 1000);
...
void NRF24L01_Check_IRQ(uint8_t *Tx_Data)
{
	 ...
	 */
	if( Status&(1<<5) ) // Daca TX_DS == 1, ACK a fost receptionat cum trebuie
	{
		NRF24L01_WriteReg(STATUS, 0b01111110);	// Write 1L to make FLAGS 0L????
		NRF24L01_Transmit (Tx_Data);
	}
	else if( Status&(1<<4) ) // Daca MAX_RT == 1, ACK NU a fost receptionat cum trebuie dupa 15 incercari
	{
		NRF24L01_WriteReg(STATUS, 0b01111110);	// Write 1L to make FLAGS 0L????
		/*
...
void NRF24L01_Init_RxAA(TIM_HandleTypeDef *htim)
{
	...
	NRF24L01_WriteReg(CONFIG, 0b00111011);  // Configure conform caietel;
	...
	NRF24L01_WriteReg(RX_PW_P1, 0b00000011);  // Seteaza Payload Width - 3 octeti
...
void NRF24L01_Receive (uint8_t *Rx_Data)
{
	CE_Disable();
	...
	// Sending the PD
	HAL_SPI_Receive(NRF24_SPI, Rx_Data, sizeof(Rx_Data), 1000);
